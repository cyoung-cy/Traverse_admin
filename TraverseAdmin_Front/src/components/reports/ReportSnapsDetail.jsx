...
import { reportsAPI, snapAPI } from "../../utils/api";
import ReportProcessModal from "./ReportProcessModal";
import Header from "../common/Header";
import ImageModal from "../common/ImageModal";

const ReportSnapsDetail = () => {
    ...
    const fetchReport = async () => {
      try {
        ...
        if (response && response.data) {
          ...
          setError(null);
          // 스냅 상세 추가 fetch (snap_id가 있으면)
          if (response.data.snap_id && snapAPI.getSnapDetail) {
            try {
              const snapRes = await snapAPI.getSnapDetail(
                  response.data.snap_id
              );
              if (snapRes && snapRes.data) {
                setSnapDetail(snapRes.data.data);
              }
            } catch {}
          }
        } else {
  ...
  const handleImageClick = (url, index, images) => {
          ...
          <div className="p-6 bg-gray-800 bg-opacity-50 rounded-lg border border-gray-700 md:col-span-2 h-fit">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-24">
                <div>
                  <h3 className="mb-1 font-medium text-gray-400">스냅 ID</h3>
                  <p className="text-sm text-gray-200">{report.snap_id}</p>
                </div>
                <div>
                  ...
                  <>
                    <div className="pb-4 border-b border-gray-700">
                      ...
                      <p className="text-sm text-gray-200 whitespace-pre-wrap max-h-[200px] overflow-y-auto">
                        {snapDetail.captions?.join(", ")}
                      </p>
                    </div>
                    <div className="pb-4 border-b border-gray-700">
                      <h3 className="mb-1 font-medium text-gray-400">스냅 사진</h3>
                      <div className="flex overflow-x-auto flex-wrap gap-2 mt-2">
                        {snapDetail.snap_images &&
                        snapDetail.snap_images.length > 0 ? (
                            snapDetail.snap_images.map((url, idx) => (
                                <img
                                    key={idx}
                                    src={url}
                                    alt={`스냅 이미지 ${idx + 1}`}
                                    ...
                                    onClick={() =>
                                        handleImageClick(url, idx, snapDetail.snap_images)
                                    }
                                />
                            ))
                        ) : (
                    ...
                    <div className="flex overflow-x-auto flex-wrap gap-4">
                      {report.evidence.map((url, idx) => (
                          <img
                              ...
                              onClick={() =>
                                  handleImageClick(url, idx, report.evidence)
                              }
                          />
                      ))}
                    </div>
...
export default ReportSnapsDetail;
