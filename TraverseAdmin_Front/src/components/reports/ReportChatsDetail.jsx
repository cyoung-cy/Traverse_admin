...
import { reportsAPI, chatAPI } from "../../utils/api";
import ReportProcessModal from "./ReportProcessModal";
import Header from "../common/Header";
import ImageModal from "../common/ImageModal";
import ChatMsgModal from "../ChatRooms/ChatMsgModal";

const ReportChatsDetail = () => {
  ...
  const [isChatMsgModalOpen, setIsChatMsgModalOpen] = useState(false);
    ...
    const fetchReportDetail = async () => {
      try {
        ...
        if (response && response && response.data) {
          ...
          setError(null);
          console.log("ReportChatsDetail 데이터:", response.data);
        } else {
  ...
  const handleShowMessage = async () => {
    try {
      console.log("report.chat_room_id:", report.chat_room_id);
      console.log("report.chat_id:", report.chat_id);

      const roomDetailResponse = await chatAPI.getChatRoomDetail(report.chat_room_id);
      const roomDetail = roomDetailResponse.data;
      console.log("roomDetailResponse:", roomDetailResponse);
      console.log("roomDetail:", roomDetail);

      if (!roomDetail || !Array.isArray(roomDetail.messages)) {
        alert("채팅 메시지를 찾을 수 없습니다. (잘못된 응답)");
        return;
      }
      ...
      if (report.chat_id) {
        msg = roomDetail.messages.find((m) => m.message_id === report.chat_id);
      }

      if (!msg) {
        msg = roomDetail.messages
            .slice()
            .sort((a, b) => b.report_count - a.report_count)[0];
      }
      console.log("선택된 메시지 msg:", msg); // 여기를 꼭 찍어보세요
      if (!msg) {
        alert("표시할 메시지를 찾을 수 없습니다.");
        return;
      }

      setChatMessage(msg);
      setIsChatMsgModalOpen(true);
    } catch (error) {
  ...
  const handleChatImageClick = (index) => {
  ...
  return (
          ...
          <div className="space-y-6 md:col-span-1">
            <div className="overflow-y-auto p-6 h-full bg-gray-800 bg-opacity-50 rounded-lg border border-gray-700">
              ...
              <div className="space-y-2">
                ...
                {report.chat_room_id && report.chat_id && (
                  <button
        ...
        <div className="p-6 bg-gray-800 bg-opacity-50 rounded-lg border border-gray-700 md:col-span-2 h-fit">
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-24">
              <div>
                <h3 className="mb-1 font-medium text-gray-400">신고 유형</h3>
                <p className="text-sm text-gray-200">{report.type ?? "-"}</p>
              </div>
              <div>
                    ...
                    <div className="flex overflow-x-auto flex-wrap gap-4">
                      {report.evidence.map((url, index) => (
                          <img
                              ...
                              onClick={() => handleChatImageClick(index)}
                          />
                      ))}
                    </div>
                  </div>
      ...
      />
        ...
        {/* 메시지 상세 모달 */}
        {isChatMsgModalOpen && chatMessage && (
            <ChatMsgModal
                isOpen={isChatMsgModalOpen}  // 여기 추가
                message={chatMessage}
                onClose={() => setIsChatMsgModalOpen(false)}
            />
...
export default ReportChatsDetail;
