...
const fetchReports = async (params) => {
  const res = await reportsAPI.getReportsChats(params);
  // mock: { success, data: { reports, total_pages, ... } }
  // axios: { data: { reports, total_pages, ... } }
  if (res.data) {
    return {
...
const ReportChatTable = () => {
  ...
  const columns = [
    { key: "report_id", label: "신고 ID" },
    {
      ...
      render: (row) => (
        <div>
          <div>방: {row.chat_room_id}</div>
          <div className="text-sm text-gray-400">메시지: {row.chat_id}</div>
        </div>
      ),
    },
    { key: "reporter_user_id", label: "신고자" },
    { key: "reported_user_id", label: "대상자" },
    {
      key: "reason",
      label: "사유",
      render: (row) => (
  ...
  return (
    <ReportTableBase
      ...
      onRowClick={(row) => navigate(`/report/chat/${row.report_id}`)}
