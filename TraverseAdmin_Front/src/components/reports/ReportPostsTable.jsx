...
const searchFilterFn = (row, searchTerm) => {
  ...
  return (
    (row.reporter_user_id &&
      row.reporter_user_id.toString().includes(searchLower)) ||
...
const fetchReports = async (params) => {
  const res = await reportsAPI.getReportsPosts(params);
  if (res.data) {
    return {
...
const ReportPostsTable = () => {
  ...
  const columns = [
    { key: "report_id", label: "신고 ID" },
    {
      ...
      render: (row) => (
        <div>
          <div>{row.title}</div>
          <div className="text-sm text-gray-400">{row.post_id}</div>
        </div>
      ),
    },
  ...
  return (
    <ReportTableBase
      ...
      onRowClick={(row) => navigate(`/report/post/${row.report_id}`)}
