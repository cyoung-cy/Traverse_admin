...
const ReportUsersDetail = () => {
  ...
  const handleProcessReport = async (reportId, processData) => {
    try {
      ...
      if (processData.userStatus && processData.targetUserId) {
        try {
          await userAPI.updateUserStatus(processData.targetUserId, {
            status: processData.userStatus,
          });
        } catch (statusError) {
          setError("사용자 계정 상태 변경 중 오류가 발생했습니다.");
  ...
  const handleUnsuspend = async (reportId, processData) => {
    try {
      ...
      if (report.target_user_id) {
        try {
          await userAPI.updateUserStatus(report.target_user_id, {
            status: "active",
          });
        } catch (statusError) {
          setError("사용자 계정 상태 변경 중 오류가 발생했습니다.");
    ...
    <div className="overflow-auto relative z-10 flex-1">
      ...
      <div className="grid grid-cols-1 gap-6 px-4 py-6 mx-auto max-w-7xl md:grid-cols-3 lg:px-8">
        ...
        <div className="space-y-6 md:col-span-1">
          <div className="p-6 h-full bg-gray-800 bg-opacity-50 rounded-lg border border-gray-700 max-h-[800px] overflow-y-auto">
        ...
        <div className="p-6 bg-gray-800 bg-opacity-50 rounded-lg border border-gray-700 md:col-span-2 h-fit">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-200 max-h-[200px] overflow-y-auto text-sm">
                {report.description ?? "-"}
              </p>
            </div>
          ...
          <div className="p-6 bg-gray-800 bg-opacity-50 rounded-lg border border-gray-700 md:col-span-3">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <p className="text-sm text-gray-200">{report.processed_by}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-200">{report.action_taken}</p>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-200">{report.comment}</p>
              </div>
            </div>
          </div>
        )}

        {/* 신고 처리 버튼: pending 상태일 때만 노출 */}
      ...
      <ReportProcessModal
        ...
        targetUserId={
          report.target_user_id ?? report.target_details?.id ?? null
        }
      />
...
export default ReportUsersDetail;
